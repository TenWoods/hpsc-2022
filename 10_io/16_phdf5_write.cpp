...
int main (int argc, char** argv) {
  ...
  hsize_t blocks[2] = {Nlocal[0]/dim[0], Nlocal[1]/dim[1]};
  hsize_t offset[2] = {mpirank / dim[0], mpirank % dim[0]};
  for(int i=0; i<2; i++) offset[i] *= blocks[i];
  hsize_t count[2] = {2,2};
  ...
  H5Sselect_hyperslab(globalspace, H5S_SELECT_SET, offset, stride, count, blocks);
  H5Pclose(plist);
